...
setwd("~/Documents/GitHub/PHP2550/Project 1")
...
# swan - p on c
# recode 1 to 0 and 2/3 to 1
swan_df <- parent_df %>%
  select(parent_id, swan1:swan18)

swan_df <- na.omit(swan_df)
...
swan_df <- swan_df %>%
  mutate(swan_inattentive = rowSums(dplyr::select(., swan1:swan9), 
         ...
         swan_hyperactive = rowSums(dplyr::select(., swan10:swan18), 
                                    na.rm=TRUE)) %>%
  select(parent_id, swan_inattentive, swan_hyperactive)
