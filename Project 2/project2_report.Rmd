...
**Background:** Bronchopulmonary dysplasia (BPD) poses a significant challenge in the care of preterm infants, with severe cases often needing tracheostomy. This study aims to develop models for predicting tracheostomy in neonates with severe BPD using respiratory parameters at 36 and 44 weeks post-menstrual age (PMA).

**Methods:** A multicenter, retrospective case-control study was conducted, involving 996 infants born at less than 32 weeks PMA. Clinical data were collected at birth, 36 weeks PMA, 44 weeks PMA, and at discharge. Multiple imputation addressed missing data. Four logistic mixed-effects models were developed, with 30% of observations reserved for validation. Models were evaluated using area under the receiver operating characteristic curve (AUC), Brier scores, precision, recall, and F1 score.

**Results:** The prevalence of tracheostomy was 15%. Respiratory support level, prenatal corticosteroids, and their interactions emerged as significant predictors. The 44-week model outperformed the 36-week model, showcasing the importance of respiratory parameters closer to discharge. Prenatal corticosteroids were associated with an increased tracheostomy risk.

**Conclusion:** Prenatal corticosteroids, respiratory support level, fraction of inspired oxygen, and medication for pulmonary hypertension were selected as predictors of tracheostomy. However, class imbalance and small sample size necessitate cautious interpretation. Future research should validate these models in larger, diverse cohorts, addressing these limitations for enhanced generalizability.
...
There were large proportions of missing data for each variable collected at the 44 week timepoint (Table 1). Although these had high proportions of missing data, the fact that each variable was also collected at 36 weeks indicates that multiple imputation may accurately predict values at 44 weeks. We thus used multiple imputation to fill in these values -- however, due to this close relationship between 36 and 44 week variables, we did not fit both timepoints in the same models. Instead, we fit separate models for the 36 week data and the 44 week data. Clinicians can then choose which model to use based on the data that they have available. 
...
# Create table with prevalence of missing data
data.frame(variable = names(colMeans(is.na(df_eda))), 
           perc_missing = paste0(round(100*colMeans(is.na(df_eda)),1),"%"), 
           row.names = NULL)  %>%
  kableExtra::kbl(format = "latex", caption = "Prevalence of Missing Data", 
...
Since we planned to account for the various centers using a mixed-effects regression model, we chose a variable selection technique that did not shrink the coefficient values. This way, we could use variable selection to identify our subset of covariates, which could then be entered into a mixed-effects model (which assumes no regularization). Although forward stepwise selection is often used as a quicker alternative to best subset selection, recent packages in R have improved the computational feasability of running best subsets. We used the package `L0Learn` to fit best subset selection on our data. The best subsets optimization problem can be expressed as so: 

$$
min \frac{1}{n} \sum_{i=1}^n (y_i - X_i\beta)^2 + \lambda\sum_{j=1}^p 1\{\beta_j \ne 0 \}
$$

## Model Development 

Thirty percent of the observations (N = 304) were set aside as a validation data set. The remaining observations (N = 692) were used to train the models. The training and validation sets each contained equal proportions of observations with a tracheostomy. We developed four types of models: (1) birth and 36-week data, including no interactions; (2) birth and 36-week data, including all interactions with ventilation support level; (3) birth and 44-week data, including no interactions; (4) birth and 44-week data, including all interactions with ventilation support level.

A best subset regression algorithm was used to select the variables included in each of the four types of models. First, fivefold cross-validation was used to choose the optimal $\lambda$ value that minimized logistic loss. Best subset models using the optimal $\lambda$ values were then fit on each of the five imputed datasets. Any variables that were selected when using at four of the five imputed datasets were included in subsequent mixed-effects models. 
...
Data were collected from ten hospital centers, with the number of observations per center ranging from just 1 patient to 630. Some centers were referral centers, which see a higher proportion of sicker patients. The proportions of each respiratory support level per center are reported in Table 3. Centers 1 and 12 had higher prevalences of subjects requiring invasive positive pressure and more subjects receiving a tracheostomy, suggesting that these may be referral centers. 
...
data.frame(covariates = c("Intercept", "Prenatal Corticosteroids", "NIPPV (36 Wks)", 
                       ...
                       model1 = kableExtra::linebreak(c("-2.34\n(SD 2.33)", 
                                                        "-", "-0.28", "1.07", 
                                                        "0.46", "-", "-",
                                                        "-", "-", "-", "-",
                                                        "-")),
                       model2 = kableExtra::linebreak(c("-3.05\n(SD 2.34)", 
                                                        "1.03", "-0.56", 
                                                        "0.81", "-0.27", 
                                                        "1.58", "0.67",
                                                        "-", "-", "-", "-",
                                                        "-")),
                       model3 = kableExtra::linebreak(c("-4.34\n(SD 2.46)", 
                                                        "1.31", "-", 
                                                        "-", "-", 
                                                        "-", "-",
                                                        "0.58", "2.60", "-", "-",
                                                        "-")),
                       model4 = kableExtra::linebreak(c("-4.56\n(SD 2.19)", 
                                                        "1.24", "-", 
                                                        "-", "-", 
                                                        "-", "-",
                                                        "0.39", "2.91", "1.13", "0.35",
...
Both 44-week models outperformed the 36-week models. We found that the 44-week variables were more highly associated with tracheostomy placement, so it makes sense that these are better predictors in the model. However, the 44-week model should still be interpreted with caution because it used a large proportion of imputed data which may not be as accurate as the 36-week data. 
...
data.frame(covariates = c("Intercept", "NIPPV (36 Wks)", "IPPV (36 Wks)",
                          "FiO2 (36 Wks)"),
           coefficients = c("-2.34 (SD 2.33)", "-0.28", "1.07", "0.46"),
           oddsratio = c("", "0.76 (0.52, 1.10)", "2.92 (2.05, 4.16)",
                         "1.58 (1.44, 1.73)"),
           p_value = c("",  "0.145", "<0.001", "<0.001")) %>%
...
Having a respiratory support level of NIPPV at 36 weeks did not have a significant effect on tracheostomy probability in comparison to needing no respiratory support. The odds ratio for IPPV at 36 weeks versus no respiratory support was 2.92, indicating that those needed IPPV support had 2.92 times higher odds of tracheostomy than those that didn't need respiratory support at 36 weeks. Additionally, a one-standard deviation increase in  FiO2 was associated with 1.58 times higher odds of tracheostomy occurring. In the observed data, FiO2 had a mean of 0.34 and a standard deviation of 0.15. . 

As an example, a patient from the average center with no respiratory support and  inspired oxygen fraction of 0.34 (average) would have a risk of tracheostomy of $e^{-2.34}/(1+e^{-2.34}) = 0.088$. A patient from the average center requiring IPPV support and an inspired oxygen fraction of 0.49 (average + 1SD) would have a tracheostomy risk of $e^{-2.34 + 1.07 + 0.46}/(1+e^{-2.34 + 1.07 + 0.46}) = 0.308$, which using the optimal threshold of 0.186 would classify them as needing a tracheostomy. 
...
data.frame(covariates = c("Intercept", "Prenatal Corticosteroids", "NIPPV (36 Wks)", "IPPV (36 Wks)"),
           coefficients = c("-4.34 (SD 2.46)", "1.31", "0.58", "2.60"),
           oddsratio = c("", "3.72 (3.03, 4.57)", "1.78 (1.53, 2.08)", "13.46 (11.74, 15.43)"),
